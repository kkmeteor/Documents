# CNB 代码仓库迁移至 Gitea 指南

> 本文档指导你将托管在 CNB（cnb.cool）上的代码仓库迁移到本地部署的 Gitea 实例，全程使用 git 命令行操作。迁移对象为通用 Git 托管源（CNB），与 TFS→Gitea 的迁移（见《TFS 代码迁移至 Gitea 完整指南》）技术路径不同：CNB 使用裸克隆 + mirror 推送，可保留完整历史。
...
当需要迁移大量仓库时，手动逐个操作效率很低。下面提供批处理脚本。
