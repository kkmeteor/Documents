...
title: "C++ 简单文件分发打包指南"
usage_scenario:
    - "将 C++ 依赖的文件（DLL、数据文件等）打包为 NuGet 包"
    - "安装后自动将文件复制到消费方输出目录"
keywords:
    - "NuGet"
    - "打包"
    ...
    - "targets"
    - "native"
source: "auto (NuGet打包发布指南C++简单版.md)"
---
...
> 若需分发**完整 C++ 原生库**（含头文件 + `.lib` 链接 + `.dll`），请参考《03-C-Cpp类库打包发布完整指南》。
...
```powershell
# 在 nuget-pack 目录下执行
nuget pack SV.MyPackage.nuspec -OutputDirectory .\packages
...
> ⚠️ **冲突提示**：本指南推送示例的 `-ApiKey` 出现 `sinounion` 与 `AzureArtifacts` 两种写法，属原文档内部不一致，实际以包源认证配置为准。
...
> 若为 HTTP 源，还需开启 `allowInsecureConnections`，见《02-HTTP源安全连接配置》。
