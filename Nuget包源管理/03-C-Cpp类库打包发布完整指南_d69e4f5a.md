---
title: "C++ / C# 类库 NuGet 打包发布完整指南"
usage_scenario:
    - "将本地 C++ 原生库（含头文件、.lib、.dll）打包为 NuGet 包"
    - "将 C# 类库打包为 NuGet 包并上传至内部源"
    - "配置消费方项目引用私有 NuGet 包"
keywords:
    - "NuGet"
    - "打包"
...
本文档说明如何将本地 **C++ 原生库**和 **C# 类库**打包为 NuGet 包（`.nupkg`），并上传至内部 NuGet 包源：

```
http://10.10.11.194:1001/v3/index.json
```

> 适用对象：需要分发**头文件 + .lib 链接库 + .dll 运行时**的完整 C++ 库。
> 若只需分发 DLL/数据文件、不涉及头文件与 .lib 链接，请参考《04-C++简单文件分发打包指南》。
...
> 若使用 HTTP 源，还需在 `<config>` 节点下开启 `allowInsecureConnections`，见《02-HTTP源安全连接配置》。
...
7. 从服务器上删除某个特定的包版本，例如删除 1.1.3
   nuget delete SV.BL.Serialization 1.1.3 `
     -Source http://10.10.11.194:1001/v3/index.json `
     -ApiKey sinounion `
     -NonInteractive

   执行成功后：1.1.3 版本被删除，1.1.2 自动成为最新版本，
   已安装 1.1.3 的项目更新时会回退到 1.1.2
```

> ⚠️ **冲突提示**：本流程中删除用 `-ApiKey sinounion`，而推送用 `AzureArtifacts`；其他文档对删除的 ApiKey 记录为 `AzureArtifacts`。认证失败时请与管理员确认实际 Key，详见《05-NuGet包清理与维护策略》。
